...
class BlockCypherWebSocketSubject extends WebSocketSubject<Object> {
  ...
  addressConfirmedTx = (address: string) => this.next({ 'event': 'confirmed-tx', 'address': address });
}
...
export class CryptoApiService {
  ...
  wsBlockchainPing$ = () => of(this.wsBlockchain()).pipe(tap(_ => _.ping()), switchMap(_ => _), take(1));
  wsBlockchainAddressLatestTransaction$ = (address: string) => of(this.wsBlockchain()).pipe(tap(_ => _.addressLatestTransaction(address)), switchMap(_ => _), take(1));

  wsBlockCypherPing$ = () => of(this.wsBlockCypher()).pipe(tap(_ => _.ping()), switchMap(_ => _), take(1));
  wsBlockCypherAddressConfirmedTx$ = (address: string) => of(this.wsBlockCypher()).pipe(tap(_ => _.addressConfirmedTx(address)), switchMap(_ => _), take(1));
