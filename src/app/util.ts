...
export function calcElementHeight(element: HTMLElement) {
  let ret = 0;
  if (element) {
    const style = !window || !window.getComputedStyle ? (<any>element).currentStyle : window.getComputedStyle(element);
...
export const toMax = <V extends any>(value: V[]) => (value || []).reduce((acc, val) => val > acc ? val : acc, <V>undefined);
export const toMin = <V extends any>(value: V[]) => (value || []).reduce((acc, val) => val < acc ? val : acc, <V>undefined);
