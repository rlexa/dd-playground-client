...
export class HackQueryEncoder extends HttpUrlEncodingCodec {
  encodeKey(value: string) {
    return super.encodeKey(value).replace(/\+/gi, '%2B');
  }
...
export const b64EncodeUnicode = (value: string) =>
  btoa(encodeURIComponent(value).replace(/%([0-9A-F]{2})/g, (match, p1) => String.fromCharCode(parseInt('0x' + p1, 16))));
...
export const getHeaders = (auth?: string, contentType?: string) => {
  let ret = new HttpHeaders().set('cache-control', 'no-cache').set('content-type', contentType || 'text/plain;charset=utf-8');
  if (auth) {
...
export const toFormUrlencoded = val =>
  Object.entries(val || {})
